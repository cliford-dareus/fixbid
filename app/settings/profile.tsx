...
function domainToForm(p: DomainProfile | null): FormProfile {
  ...
  return {
    ...
    website: p.website || '',
    tagline: p.tagline || '',
    defaultInclusions: p.default_inclusions || '',
    defaultExclusions: p.default_exclusions || '',
    warrantyText: p.warranty_text || '',
    depositPercent: String(p.deposit_percent ?? 50),
    quoteValidDays: String(p.quote_valid_days ?? 30),
  };
}
...
function formToDomain(f: FormProfile): Partial<DomainProfile> {
  return {
    ...
    default_inclusions: f.defaultInclusions.trim() || null,
    default_exclusions: f.defaultExclusions.trim() || null,
    warranty_text: f.warrantyText.trim() || null,
    deposit_percent: parseInt(f.depositPercent, 10) || 50,
    quote_valid_days: parseInt(f.quoteValidDays, 10) || 30,
  };
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <SectionCard title="Client quote defaults" colors={colors}>
          <Text className="text-muted-foreground mb-3 text-[12px] leading-4">
            Shown on the public quote link so clients know what is included, what is not, and your warranty.
          </Text>
          {editing ? (
            <>
              <Field
                label="Included (one per line)"
                value={draft.defaultInclusions}
                onChange={update('defaultInclusions')}
                placeholder={'Labor and materials listed\nCleanup of work area\nHaul-away of debris'}
                multiline
                colors={colors}
              />
              <Field
                ...
                onChange={update('defaultExclusions')}
              ...
              <Field
                ...
                onChange={update('warrantyText')}
                placeholder="90-day workmanship warranty on labor"
                colors={colors}
              />
              <View className="flex-row gap-2">
                <View className="flex-1">
                  <Field
                    ...
                    onChange={update('depositPercent')}
                    placeholder="50"
                ...
                <View className="flex-1">
                  <Field
                    ...
                    onChange={update('quoteValidDays')}
                    placeholder="30"
                    keyboardType="number-pad"
                    colors={colors}
                  />
            ...
            <>
              {view.defaultInclusions ? (
                <InfoRow icon="check-circle" label="Included" value={view.defaultInclusions} colors={colors} />
              ) : null}
              {view.defaultExclusions ? (
                <InfoRow icon="minus-circle" label="Not included" value={view.defaultExclusions} colors={colors} />
              ) : null}
              <InfoRow
                icon="shield"
                label="Warranty"
                value={view.warrantyText || 'Not set'}
                colors={colors}
              />
              <View className="mb-1 mt-2 flex-row gap-2.5">
                <DefaultStat label="Deposit" value={`${view.depositPercent}%`} colors={colors} />
                <DefaultStat label="Valid" value={`${view.quoteValidDays} days`} colors={colors} />
              </View>
            </>
          )}
        </SectionCard>
